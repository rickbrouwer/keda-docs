...
**Example with `lookAhead: 5m`:**

| Time | Event |
|------|-------|
| t=0m | First forecast created, covers `[t+0m .. t+10m]`, target = `t+5m` |
| t=5m | Renewal triggered, new forecast covers `[t+5m .. t+15m]` |
| t=5m–t+5m30s | Previous forecast used as fallback while new one is indexed |
| t=10m | First forecast expires, Elasticsearch removes documents |
...
- The used user requires authorization `manage_ml`. See also [here](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-ml-forecast#topic-required-authorization).
