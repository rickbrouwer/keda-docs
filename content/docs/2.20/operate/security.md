...
## Restrict Which Scaler Types May Be Used

Cluster operators that only rely on a small subset of the built-in scalers can pass the `--enabled-scalers` flag to the operator to restrict which trigger types are accepted. Disabled scaler types are rejected by the admission webhook at apply time and again at scaler-build time, so users cannot accidentally create a `ScaledObject` that uses a scaler your team has not vetted. See [Restrict Enabled Scaler Types](./cluster.md#restrict-enabled-scaler-types) for usage and an important caveat about vendor code that remains in the binary.
